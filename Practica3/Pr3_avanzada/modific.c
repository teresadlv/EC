...
void charge_pin()
 {
    ...
    // se configura con la entrada ACLK (TASSEL_1) divisor entre 8 (ID_3) modo continuo (MC_2) y rst a 0 (TACLR)
    TA2CTL = TASSEL_1 | ID_3 | MC_2 | TACLR;
}
...
#pragma vector=PORT2_VECTOR
    ...
    {
        unsigned int time_elapsed = TA2R; // Recojo el valor del timer TA2R
        if (time_elapsed < 100){
            if (P2IFG & BIT0){          // detectar pulsacion
                __delay_cycles(10);
                i = 0;
                P9OUT ^= BIT7;
                display_num_lcd(i);
        ...
        P2IFG &= ~BIT0;     //limpia bit pendiente
        charge_pin();
    }
