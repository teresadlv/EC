...
__interrupt void ADC12ISR(void)
{
    ...
    {
        // Lee el valor de la temperatura en crudo (número entero sin signo)
        unsigned int temp_adc = ADC12MEM0;

        // Convierte ese valor a coma flotante utilizando la fórmula
        float temp_true = (float)(((long)temp_adc - CALADC12_12V_30C) * 55) / (CALADC12_12V_85C - CALADC12_12V_30C) + 30.0;

        // Envía ese valor a la función load_send_buffer
        load_send_buffer(temp_true);

        // Empieza la conversión en el registro ADC12CTL0
        ADC12CTL0 |= ADC12ENC | ADC12SC;
    }
    ADC12IV = ADC12IV_NONE; // Borra interrupción pendiente
}
